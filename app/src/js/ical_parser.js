 ...
 * @version: 0.2
 */
function ical_parser(feed_url,post, callback){
	 ...
	 */
	this.loadFile = function(url,post, callback){
	...
	this.parseICAL = function(data){
		...
		for(var i=0;i<cal_array.length;i++){
			...
			if(in_event){
				...
				if(type.includes("DTSTART"))type="DTSTART";
				if(type.includes("DTEND"))type="DTEND";
	 ...
	 * @param ical file url
	 */
	this.load = function(ical_file,post){
		var tmp_this = this;
		this.raw_data = null;
		this.loadFile(ical_file,post, function(data){
	...
	this.post = post;
	//Load the file
	this.load(this.feed_url,this.post);
}
